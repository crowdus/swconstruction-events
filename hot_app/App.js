...
export class User {
    constructor(userID, username, firstname, lastname, email, datejoined, password, friends) {
        if (userID.match(/^[0-9a-zA-Z]/)){ //do we need this? probably not -- it's in the database too
          if(!finduser(userID)){ //
            this.userID = userID;
          }
        }
    ...
    setUserID(_userID) { //userIDs must use only alphanumerical and have at least one number and one alphabetical number
      if (!_userID.match(/^[0-9a-zA-Z]/)){ //do we need this? probably not -- it's in the database too
        return false;
      }
      if(!finduser(_userID)){ //
